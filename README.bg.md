...
- **Прилепване в ъгли** — `Ctrl+Q` цикли ГЛ → ГД → ДЛ → ДД
...
| Скрипт | Поведение |
|---|---|
| `run.vbs` | Скрит (само в трея) |
| `run.bat` | Видим (отворена конзола) |
...
| **Прилепване в ъгъл** | `Ctrl+Q` — цикли Горе-Ляво → Горе-Дясно → Долу-Ляво → Долу-Дясно |
...
Основни стойности по подразбиране:
...
```
saipenview/
├── app.py              Свързване на компонентите — трей, бързи клавиши, прозорец, api
├── api.py              pywebview мост към JS (30+ метода)
