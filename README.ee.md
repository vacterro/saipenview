...
- **Agent Engine** — `claude-code` (või muud mootorid: codex, aider, gemini, cline, goose, agy, generic_cli) käivitamine projektis
  ...
  - **Väljundkonsool** — puhverdatud agendi väljund (vaikimisi 5000 rida), stdin-sisend (ainult mootorite puhul, mille stdin-tugi on tõestatud)
  - **Kill / stop all** — projekti tapmine ja globaalne peatus
  - **Generic CLI on käsureakäsk** — `generic-cli` käivitab instruktsiooni käsuga `cmd.exe /d /s /c` projektiga töökataloogina; jutumärgid, torud (`|`) ja `&&` töötavad. Saatmist ei pakuta ühekordsetele mootoritele (codex exec, gemini --prompt, opencode run, ...), kuni stdin-tugi on tõestatud
  - **`engine_overrides`** — mootoripõhised `{"path": <exe>, "extra_args": [...], "env": {...}}`, valideeritud enne käivitust (config.py)
...
`theme` on sümbol `saipenview/assets/themes/` kaustast — rakenduses on 16 paletti (Dark Golden, Claude Code, Antigravity, K-Lite, FreeBuff, CodeNomad, Default, Golden Vintage, Golden Default, Vintage Dark, Vintage Classic, OLED, Dracula, Nord, Solarized Dark, Custom). Vahetamine toimub kohe, taaskäivitust pole vaja: palett rakendatakse CSS-muutujatena käitusajal, nii et `style.css` värvi muutmiseks kunagi ümber ei kirjutata. `goldendefault` taasesitab täpselt stiililehe vaikeväärtused ja tundmatu sümbol langeb sellele tagasi.
...
### Teede turvalisus ja dry-run

Rakenduse salvestatud teed — `scan_roots`, `pinned_roots`, `hidden_roots`, `selected_root` —
hoitakse **kanoonilises vormis**: absoluutsed, suurtähtede suhtes normaliseeritud, sümbollingid
lahendatud, ühe lõpueraldajaga kettajuurtel. Kaust, mis lisatakse kujul `v:\projects/`,
salvestatakse ja võrreldakse kui `V:\projects`, nii et sama tee erinevad kirjapildid ei ilmu
kunagi kahe kirjena.

Skannimisjuured, mis osutavad puuduvale kettale või kaustale, **pannakse karantiini, mitte ei
kustutata**: need jäävad skannimisvea logisse ja nimekirja, nii et ketta naastes võtab järgmine
skann selle automaatselt üles.

Sisseehitatud failivaatleja avab ainult `.md`/`.json` faile, mis asuvad teadaoleva projekti juure
sees — tee, mis väljub kõigist juurtest või kannab muud laiendit, lükatakse tagasi Pythoni poolel,
mitte lihtsalt peidetakse liideses.

Kontrolli konfigi ja teekihte ilma rakendust käivitamata:

```
python -m saipenview --dry-run
```

Väljumiskood `0` — konfig puhas, `1` — puuduvad/karantiinis juured või kanoonilise vormi
lahknevus, igaüks nimetatud stdoutis.
...
│       └── app.js      Frontendi loogika (~4.3k rida)
├── assets/
│   └── tray_icon.png
