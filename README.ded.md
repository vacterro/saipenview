...
- **Agent Engine** — запуск `claude-code` (и codex, aider, gemini, cline, goose, agy, generic_cli) в проекте
  ...
  - **Консоль** — вывод агента (по умолчанию 5000 строк), ввод в stdin только для движков с подтверждённой поддержкой stdin
  - **Kill / stop all** — прибить процесс и глобальный стоп
  - **Kill / stop all** — убить процесс в проекте и глобальная остановка
  - **Generic CLI — это команда оболочки** — `generic-cli` гонит инструкцию через `cmd.exe /d /s /c` с проектом в качестве рабочего каталога; кавычки, пайпы (`|`) и `&&` работают. Кнопки «Отправить» нет у одноразовых движков (codex exec, gemini --prompt, opencode run, ...), пока stdin не докажет, что умеет
  - **`engine_overrides`** — обходы по-движковые `{"path": <exe>, "extra_args": [...], "env": {...}}`, проверяются до запуска (config.py)
...
- **Привязка к углам** — `Ctrl+Q` гоняет по кругу ЛВ → ЛН → ПН → ПВ
...
| **Привязка к углу** | `Ctrl+Q` — цикл Верх-Лево → Верх-Право → Низ-Лево → Низ-Право |
...
`theme` — это слаг из `saipenview/assets/themes/` — в приложении 16 палитр (Dark Golden, Claude Code, Antigravity, K-Lite, FreeBuff, CodeNomad, Default, Golden Vintage, Golden Default, Vintage Dark, Vintage Classic, OLED, Dracula, Nord, Solarized Dark, Custom). Переключение мгновенное, перезапуск не нужен: палитра применяется через CSS-переменные на лету, так что `style.css` ради смены цвета никогда не переписывается. `goldendefault` один в один повторяет значения таблицы стилей по умолчанию, а неизвестный слаг откатывается к нему.
...
### Безопасность путей и dry-run

Хранимые приложением пути — `scan_roots`, `pinned_roots`, `hidden_roots`, `selected_root` —
приводятся к **канонической форме**: абсолютные, нормализованные по регистру, с резолвом
символических ссылок, один завершающий разделитель на корнях дисков. Папка, добавленная как
`v:\projects/`, хранится и сравнивается как `V:\projects`, так что слеш/регистр/дубли одного
пути никогда не становятся двумя записями.

Корни сканирования на отсутствующий диск или папку **уходят в карантин, а не выбрасываются**:
они в журнале ошибок скана и остаются в списке, так что когда диск вернётся, следующий скан
подхватит его сам.

Встроенный просмотрщик файлов открывает только `.md`/`.json`, лежащие внутри известного корня
проекта — путь, вылезающий за все корни или с другим расширением, отклоняется на стороне
Python, а не просто прячется в интерфейсе.

Проверить конфиг и слои путей без запуска приложения:

```
python -m saipenview --dry-run
```

Код выхода `0` — конфиг чист, `1` — отсутствующие/карантинные корни или расхождение
канонической формы, каждое названо в stdout.
...
├── zone_picker.py      Оверлей привязки к углу Ctrl+Q (tkinter)
