...
- **محاذاة الزوايا** — `Ctrl+Q` ينقل بين أعلى اليسار ← أعلى اليمين ← أسفل اليسار ← أسفل اليمين
...
| السكريبت | السلوك |
|---|---|
| `run.vbs` | مخفي (في الصينية فقط) |
| `run.bat` | مرئي (مع فتح وحدة التحكم) |
...
| **محاذاة الزاوية** | `Ctrl+Q` — التنقل: أعلى اليسار ← أعلى اليمين ← أسفل اليسار ← أسفل اليمين |
...
```
saipenview/
├── app.py              ربط نقطة الدخول — صينية النظام، الاختصارات، النافذة، API
├── api.py              جسر pywebview المواجه لـ JS (أكثر من 30 طريقة)
