...
- **Coordinated protocol mutations** — `protocol_write.py` is the one write coordinator for every `.saipen/` mutation: per-root lock, optimistic fingerprint/CAS (an external change between read and commit is a controlled conflict, never a lost update), and the only E-/T- id allocators in the codebase. Multi-file operations re-validate every canonical input before commit, so an external edit to STATE mid-transaction aborts the whole write. Direct mutation is refused while a launched agent owns the project, and the launch reservation shares the same per-root lock — check-then-mutate and check-then-launch are one atomic decision, not a TOCTOU window. Ticket moves follow the protocol's strict state machine (TODO→start→DOING, DOING→done→DONE only with `| verify:` evidence, etc.), and outbox collect runs the current package gate (exact `ready`, every handoff field, source_head + tree fingerprint + role_revision current) before any main-project write.
